...
const Boss = ({ boss, isTurn, dialog, effectClass, cyclingDialog, gameType, damageEventId }) => {
    ...
    return (
        <div className={`character boss ${isTurn ? 'turn-active' : ''} ${effectClass} ${gameType !== 'ultimate' ? 'mini-boss-size' : ''}`}>
            <AttackEffect trigger={boss.last_damage_taken} damageEventId={damageEventId} />
            <DamageIndicator damage={boss.last_damage_taken} characterName={boss.name} damageEventId={damageEventId} />
            <DialogBubble message={dialog} duration={6000} /> {/* For attack message */}
